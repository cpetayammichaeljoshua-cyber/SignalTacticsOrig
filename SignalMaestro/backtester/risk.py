...
class RiskManager:
    ...
    def calculate_position_size(self, signal: Dict[str, Any], leverage: int, stop_loss_percentage: float = 1.5) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.use_fixed_risk:
                # Use fixed percentage of initial capital only
                base_risk = self.initial_capital * (self.risk_percentage / 100)
                # Cap maximum risk per trade at 2% of initial capital
                risk_amount = min(base_risk, self.initial_capital * 0.02)
            else:
                # Even for non-fixed, limit to conservative percentage
                risk_amount = min(
                    self.current_capital * (self.risk_percentage / 100),
                    self.initial_capital * 0.03  # Never risk more than 3% of initial
                )
            ...
            # Calculate realistic position size with portfolio limits
            stop_loss_distance_pct = entry_price * (stop_loss_percentage / 100)

            # Use percentage-based sizing for more accurate risk control
            position_value = risk_amount / stop_loss_distance_pct
            position_size = position_value / entry_price

            # Apply leverage to position size (not risk calculation)
            leveraged_position_size = position_size * leverage

            # Portfolio risk limits
            max_position_value = self.current_capital * 0.20  # Max 20% of capital per trade
            if leveraged_position_size * entry_price > max_position_value:
                leveraged_position_size = max_position_value / entry_price

            # Minimum position size check (more realistic)
            min_position_value = 5.0  # $5 minimum
            if leveraged_position_size * entry_price < min_position_value:
                leveraged_position_size = min_position_value / entry_price
