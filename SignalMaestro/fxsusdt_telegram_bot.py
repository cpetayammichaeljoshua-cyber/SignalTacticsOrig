...
*Cross Margin & Auto Leverage - Comprehensive Risk Management*
    ...
    async def process_signals(self, signals):
        ...
        for signal in signals:
            try:
                # CONFIDENCE FILTER - Allow signals with reasonable confidence
                confidence_threshold = 60.0  # Reduced from 75% to 60%

                if signal.confidence < confidence_threshold:
                    self.logger.warning(f"🚫 TRADE BLOCKED - Signal confidence {signal.confidence:.1f}% below 60% threshold")
                ...
                if self.ai_processor:
                    ...
                    # Check AI confidence
                    ai_confidence = enhanced_signal.get('ai_confidence', 0.75) if enhanced_signal else 0.75
                    if ai_confidence < 1.0:
                        ai_confidence_pct = ai_confidence * 100
                    else:
                        ai_confidence_pct = ai_confidence

                    if enhanced_signal and ai_confidence_pct >= confidence_threshold:
                        self.logger.info(f"✅ TRADE APPROVED - Signal confidence {signal.confidence:.1f}%, AI confidence {ai_confidence_pct:.1f}%")
                        await self.send_enhanced_signal(enhanced_signal)
                    else:
                        self.logger.warning(f"🤖 AI BLOCKED signal - AI confidence {ai_confidence_pct:.1f}% below {confidence_threshold}% threshold")
                else:
    ...
    async def send_enhanced_signal(self, enhanced_signal: Dict[str, Any]) -> bool:
        """Send enhanced signal to channel"""
        try:
            # Extract signal data
            symbol = enhanced_signal.get('symbol', 'FXSUSDT')
            action = enhanced_signal.get('action', 'BUY')
            entry_price = enhanced_signal.get('entry_price', 0)
            
            # Create Ichimoku signal object for formatting
            ...
            signal = IchimokuSignal(
                ...
                stop_loss=enhanced_signal.get('stop_loss', entry_price * 0.98),
                take_profit=enhanced_signal.get('take_profits', [entry_price * 1.03])[0],
                signal_strength=enhanced_signal.get('ai_signal_strength', 75),
                confidence=enhanced_signal.get('ai_confidence', 0.75) * 100,
                risk_reward_ratio=enhanced_signal.get('risk_reward_ratio', 1.5),
                atr_value=enhanced_signal.get('atr_value', 0.001),
                timestamp=datetime.now(),
                timeframe="AI_Enhanced"
            )
            
            # Send to channel
            return await self.send_signal_to_channel(signal)
            
        except Exception as e:
            self.logger.error(f"Error sending enhanced signal: {e}")
            return False
