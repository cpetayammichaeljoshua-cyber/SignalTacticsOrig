...
from SignalMaestro.zecusdt_trader_adapter import ZECUSDTTrader
from SignalMaestro.telegram_channel_scanner import TelegramChannelScanner

class ZECUSDTTelegramBot:
    """Telegram bot for ZECUSDT.P signals"""
    ...
    def __init__(self):
        ...
        self.trader = ZECUSDTTrader() # ZEC/USDT specific trader

        # Telegram Channel Scanner
        self.channel_scanner = TelegramChannelScanner(self.bot_token, channel_id="3464978276")
        self.scanned_channel_signals = []
        ...
        self.commands = {
            ...
            '/channel_status': self.cmd_channel_status
        }
        ...
        # ZECUSDT contract specifications
        self.contract_specs = {
            'symbol': 'ZECUSDT',
            'base_asset': 'ZEC',
            'quote_asset': 'USDT',
        ...
        self.logger.info("🤖 ZECUSDT Futures Telegram Bot initialized with advanced commands")
...
*Cross Margin & Auto Leverage - Comprehensive Risk Management*
    ...
    async def send_signal_to_channel(self, signal: IchimokuSignal) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(f"📡 Signal sent to {self.channel_id}: {signal.action} ZECUSDT.P @ {signal.entry_price:.5f}")

                # Send to admin if configured
                if self.admin_chat_id:
                    admin_msg = f"✅ Signal sent to {self.channel_id}\n{signal.action} ZECUSDT.P @ {signal.entry_price:.5f}"
    ...
    async def send_status_update(self, message: str) -> bool:
        ...
        if self.admin_chat_id:
            return await self.send_message(self.admin_chat_id, f"🤖 **ZECUSDT Bot Status**\n\n{message}")
        return True
    ...
    async def scan_telegram_channel(self) -> bool:
        """Scan Telegram channel for trading signals"""
        try:
            signals = await self.channel_scanner.scan_for_trades(limit=20)
            
            if not signals:
                return False
            
            self.logger.info(f"📡 Found {len(signals)} signals from channel 3464978276")
            
            # Process each channel signal
            for channel_signal in signals:
                try:
                    # Validate signal
                    if not await self.channel_scanner.validate_signal(channel_signal):
                        self.logger.debug(f"⚠️ Channel signal validation failed")
                        continue
                    
                    # Check rate limiting
                    if not self.can_send_signal():
                        self.logger.info("⏸️ Rate limit active, skipping channel signal")
                        continue
                    
                    # Format and send signal
                    formatted_msg = self.channel_scanner.format_signal_for_trading(channel_signal)
                    success = await self.send_message(self.channel_id, f"📡 **CHANNEL SIGNAL**\n\n{formatted_msg}")
                    
                    if success:
                        self.signal_count += 1
                        self.last_signal_time = datetime.now()
                        self.signal_timestamps.append(self.last_signal_time)
                        self.logger.info(f"✅ Channel signal relayed: {channel_signal['direction']} {channel_signal['symbol']}")
                
                except Exception as e:
                    self.logger.error(f"Error processing channel signal: {e}")
                    continue
            
            return True
        
        except Exception as e:
            self.logger.error(f"Error scanning Telegram channel: {e}")
            return False

    async def scan_and_signal(self) -> bool:
        """Enhanced multi-timeframe scanning for increased trade frequency"""
        try:
            self.logger.info("🔍 Scanning ZECUSDT.P for Ichimoku signals (Multi-TF)...")
...
{direction_emoji} **ZECUSDT.P** hit your target price!
...
Use `/alerts` to manage your alerts."""
    ...
    async def run_continuous_scanner(self):
        """Run continuous market scanner with Telegram channel monitoring"""
        self.logger.info("🚀 Starting ZECUSDT.P continuous scanner with channel monitoring...")
        ...
        await self.send_status_update("🚀 ZECUSDT.P Ichimoku Sniper Bot started\n📊 Monitoring 30-minute timeframe\n📡 Channel Scanner: 3464978276\n🎯 Ready for signals")
        ...
        channel_scan_interval = 45  # 45 seconds for channel
        ...
        try:
            ...
            while True:
                try:
                    ...
                    # Channel scan (every 3rd cycle to avoid API spam)
                    channel_scan_counter += 1
                    if channel_scan_counter >= 3:
                        await self.scan_telegram_channel()
                        channel_scan_counter = 0
            ...
            await self.send_status_update("🛑 ZECUSDT.P Bot stopped")
        except Exception as e:
    ...
    async def cmd_start(self, update, context):
        ...
        await self.send_message(chat_id, "Welcome to the ZECUSDT.P Futures Bot! Type /help for a list of commands.")
    ...
    async def cmd_status(self, update, context):
        ...
        status_message = (
            f"🤖 **ZECUSDT.P Futures Bot Status:**\n\n"
            ...
            f"• **Contract:** `ZECUSDT.P (Perpetual Futures)`\n"
    ...
    async def cmd_price(self, update, context):
        """Get the current price of ZECUSDT.P"""
        chat_id = str(update.effective_chat.id)
        try:
            ...
            if price:
                ...
                ticker = await self.trader.get_24hr_ticker_stats('ZECUSDT')
                if ticker:
                    ...
                    message = f"""💰 **ZECUSDT.P Price Information:**
...
**📈 Contract:** ZECUSDT Perpetual"""
                else:
                    message = f"💰 **Current ZECUSDT.P Price:** `{price:.5f}`"

                await self.send_message(chat_id, message)
            else:
                await self.send_message(chat_id, "❌ Could not retrieve ZECUSDT.P price.")
        except Exception as e:
    ...
    async def cmd_balance(self, update, context):
        ...
        try:
            ...
            if balance:
                message = f"""💰 **Account Balance (ZECUSDT Futures):**
...
**⚡ Updated:** {datetime.now().strftime('%H:%M:%S UTC')}"""
    ...
    async def cmd_position(self, update, context):
        """Get current open positions for ZECUSDT.P"""
        chat_id = str(update.effective_chat.id)
        try:
            positions = await self.trader.get_positions('ZECUSDT')
            if positions:
                message = "📊 **Open Positions (ZECUSDT.P):**\n\n"
...
"""
                ...
                await self.send_message(chat_id, "ℹ️ You have no open positions for ZECUSDT.P.")
        except Exception as e:
    ...
    async def cmd_market(self, update, context):
        ...
        chat_id = str(update.effective_chat.id)
        symbol = 'ZECUSDT' # Default to ZECUSDT
        if context.args:
...
**⏰ Last Update:** `{datetime.now().strftime('%H:%M:%S UTC')}`"""
    ...
    async def cmd_leverage(self, update, context):
        """Get or set leverage for ZECUSDT.P with dynamic calculation"""
        chat_id = str(update.effective_chat.id)
        ...
        if len(context.args) >= 1 and context.args[0].upper() == 'AUTO':
            ...
            try:
                ...
                atr_data = await position_manager.calculate_multi_timeframe_atr('ZECUSDT')

                # Detect market regime
                market_regime = await position_manager.detect_market_regime('ZECUSDT')

                # Calculate optimal leverage
                optimal_leverage = await position_manager.calculate_optimal_leverage(
                    'ZECUSDT', atr_data, market_regime, account_balance
                )
...
Use `/leverage ZECUSDT {optimal_leverage}` to apply this leverage."""
        ...
        if len(context.args) >= 2 and context.args[0].upper() == 'ZECUSDT':
            symbol = context.args[0].upper()
            try:
                leverage = int(context.args[1])
                if 1 <= leverage <= 50: # Max leverage is 50x for ZECUSDT
                    ...
                    await self.send_message(chat_id, "❌ Leverage must be between 1x and 50x for ZECUSDT.")
            except ValueError:
            ...
            try:
                current_leverage = await self.trader.get_leverage('ZECUSDT')
                if current_leverage:
...
**Usage:** `/leverage ZECUSDT <1-50>` to change leverage""")
    ...
    async def cmd_risk(self, update, context):
        ...
        try:
            ...
            risk_calc = f"""🎯 **Risk Calculation for ZECUSDT.P**
...
🕐 **Time:** {datetime.now().strftime('%H:%M:%S UTC')}""")
    ...
    async def cmd_history(self, update, context):
        ...
        try:
            ...
            trades = await self.trader.get_trade_history('ZECUSDT', limit=10)

            if not trades:
                await self.send_message(chat_id, "📜 **Trade History**\n\nNo recent trades found for ZECUSDT.P")
                self.commands_used[chat_id] = self.commands_used.get(chat_id, 0) + 1
                return

            history_msg = "📜 **Recent Trade History (ZECUSDT.P)**\n\n"
...
💡 **Note:** This shows executed trades only. Use `/position` for current open positions."""
            ...
            fallback_msg = f"""📜 **Trade History (ZECUSDT.P)**
...
• `/scan` - Manual signal scan"""
    ...
    async def cmd_alerts(self, update, context):
        ...
        if not context.args:
            ...
            if not user_alerts:
                alerts_msg = """🔔 **Price Alerts for ZECUSDT.P**
...
• `/alerts remove 1` - Remove first alert"""
                ...
                alerts_msg = "🔔 **Active Price Alerts (ZECUSDT.P):**\n\n"
...
📊 **Remaining Alerts:** {len(user_alerts)}/5""")
    ...
    async def cmd_admin(self, update, context):
        ...
        try:
            if not context.args:
                # Show admin panel
                admin_panel = f"""👑 **ZECUSDT.P Admin Panel**
...
• ✅ Intelligent Updates"""
    ...
    async def cmd_futures_info(self, update, context):
        """Get general information about ZECUSDT.P futures contract"""
        chat_id = str(update.effective_chat.id)
        info = (
            "ℹ️ **ZECUSDT.P Futures Contract Information:**\n\n"
    ...
    async def cmd_contract_specs(self, update, context):
        """Get detailed contract specifications for ZECUSDT.P"""
        chat_id = str(update.effective_chat.id)
        specs = (
            "📜 **ZECUSDT.P Contract Specifications:**\n\n"
            f"• **Symbol:** `{self.contract_specs['symbol']}`\n"
            ...
            "ZECUSDT.P is a futures contract, emphasizing its use in derivatives trading."
        )
        await self.send_message(chat_id, specs)
        self.commands_used[chat_id] = self.commands_used.get(chat_id, 0) + 1

    async def cmd_funding_rate(self, update, context):
        """Get the current funding rate for ZECUSDT.P"""
        chat_id = str(update.effective_chat.id)
        try:
            funding_rate_data = await self.trader.get_funding_rate('ZECUSDT')
            if funding_rate_data:
                ...
                message = (
                    f"💸 **Current Funding Rate (ZECUSDT.P):** `{rate}`\n"
                    f"Next Funding Payment: {next_funding_time}"
                )
                await self.send_message(chat_id, message)
            else:
                await self.send_message(chat_id, "❌ Could not retrieve funding rate for ZECUSDT.P.")
        except Exception as e:
    ...
    async def cmd_open_interest(self, update, context):
        """Get the current open interest for ZECUSDT.P"""
        chat_id = str(update.effective_chat.id)
        try:
            oi_data = await self.trader.get_open_interest('ZECUSDT')
            if oi_data:
                oi = oi_data['openInterest']
                amount_in_quote_asset = oi_data.get('totalQuoteAssetVolume', 'N/A') # May not always be available
                message = (
                    f"📊 **Open Interest (ZECUSDT.P):**\n"
                    f"• **Contracts:** `{oi}`\n"
                )
                ...
                await self.send_message(chat_id, "❌ Could not retrieve open interest for ZECUSDT.P.")
        except Exception as e:
    ...
    async def cmd_volume_analysis(self, update, context):
        """Get recent trading volume for ZECUSDT.P"""
        chat_id = str(update.effective_chat.id)
        try:
            ...
            if klines:
                ...
                message = (
                    f"📈 **Volume Analysis (ZECUSDT.P - Last 25 Hours):**\n\n"
    ...
    async def cmd_market_sentiment(self, update, context):
        ...
        sentiment_message = (
            "🤔 **Market Sentiment Analysis (ZECUSDT.P):**\n\n"
    ...
    async def cmd_market_news(self, update, context):
        """Fetch recent news relevant to ZECUSDT.P or crypto markets"""
        chat_id = str(update.effective_chat.id)
        ...
        try:
            ...
            if ticker:
                ...
                if abs(change_percent) > 5:
                    volatility_news = f"🚨 **High Volatility Alert:** ZECUSDT.P moved {change_percent:+.2f}% in 24h"
                elif abs(change_percent) > 2:
                    volatility_news = f"📊 **Moderate Movement:** ZECUSDT.P showing {change_percent:+.2f}% change"
                else:
                    volatility_news = f"📈 **Stable Trading:** ZECUSDT.P consolidating with {change_percent:+.2f}% change"

                volume_analysis = "🔥 High volume" if volume > 1000000 else "📊 Normal volume" if volume > 500000 else "💤 Low volume"

                news_message = f"""📰 **ZECUSDT.P Market News & Analysis**
...
**🔍 Market Factors:**
• **DXY Influence:** USD strength affects ZECUSDT movement
...
• `/dynamic_sltp SHORT` - Calculate smart SL/TP for short position""")
            ...
            smart_system = get_smart_sltp_system('ZECUSDT')
...
"""
            ...
            sl_tp_config = await position_manager.calculate_dynamic_sl_tp(
                'ZECUSDT', direction, current_price, atr_data, market_regime
            )
    ...
    async def cmd_market_dashboard(self, update, context):
        ...
        try:
            ...
            if ticker:
                ...
                dashboard = f"""📊 **ZECUSDT Market Dashboard**
...
• Tenkan: 9, Kijun: 26, Senkou B: 52
    ...
    async def cmd_channel_status(self, update, context):
        """Get Telegram channel scanner status"""
        chat_id = str(update.effective_chat.id)
        try:
            status_msg = f"""📡 **Telegram Channel Scanner Status**

📍 **Channel:** 3464978276
🟢 **Status:** Active

**Statistics:**
• **Messages Scanned:** {len(self.channel_scanner.scanned_messages)}
• **Signals Detected:** {len(self.scanned_channel_signals)}
...
✅ Price level parsing (Entry, SL, TP)
✅ Leverage detection
✅ Confidence scoring (75% minimum)

🎯 **Integration:** Signals from channel 3464978276 are automatically processed and relayed as trades on ZEC/USDT futures."""
            await self.send_message(chat_id, status_msg)
        except Exception as e:
            await self.send_message(chat_id, f"❌ Error getting channel status: {e}")
        self.commands_used[chat_id] = self.commands_used.get(chat_id, 0) + 1
    ...
    async def start_telegram_polling(self):
        ...
        try:
            ...
            await self.send_status_update("🚀 ZECUSDT.P Futures Bot commands are now active!")
...
async def main():
    """Main function to run the ZECUSDT bot"""
    logging.basicConfig(
        level=logging.INFO,
