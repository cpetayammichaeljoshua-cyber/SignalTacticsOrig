...
class HighFrequencyScalpingOrchestrator:
    ...
    def __init__(self, signal_callback=None):
        ...
        # Signal callback for external integrations (Telegram, etc.)
        ...
        try:
            ...
            if signal:
                ...
                if position_manager:
                    ...
                    self.logger.info(f"   Expected Duration: {signal.expected_duration_seconds}s")
                    self.logger.info(f"   Signal Latency: {signal.signal_latency_ms:.1f}ms")

                    # Push signal to Telegram if callback is set
