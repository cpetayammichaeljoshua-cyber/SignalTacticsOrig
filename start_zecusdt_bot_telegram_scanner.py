#!/usr/bin/env python3
"""
Start ZECUSDT Bot with Telegram Channel Scanning
Scans channel 3464978276 for trade signals and executes on ZEC/USDT
"""
...
try:
    from SignalMaestro.zecusdt_telegram_bot import ZECUSDTTelegramBot
...
async def main():
    """Main function with Telegram channel scanning"""
    logging.basicConfig(
        level=logging.INFO,
    ...
    logger.info("=" * 80)
    logger.info("🚀 ZECUSDT.P TELEGRAM SCANNER BOT STARTING")
    logger.info("=" * 80)
    logger.info(f"📊 Symbol: ZECUSDT.P (Perpetual Futures)")
    logger.info(f"📡 Scanning Channel: 3464978276")
    logger.info(f"⏱️ Timeframe: 30 minutes")
    logger.info(f"🎯 Strategy: Ichimoku Sniper + Telegram Channel Integration")
    ...
    logger.info("   • /channel_status - Telegram channel scanner status")
    logger.info("=" * 70)

    # Start continuous scanner with channel monitoring
