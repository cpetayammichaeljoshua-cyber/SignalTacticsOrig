...
class TelegramSignalNotifier:
    ...
    def _format_signal_message(self, signal: Any) -> str:
        ...
        # Build STRICT CORNIX FORMAT with comprehensive strategy details
        # SECTION 1: Strategy Intelligence Details
        strategy_lines = [
        ...
        # SECTION 2: OFFICIAL CORNIX SIGNAL FORMAT
        cornix_lines = [
            f"",
            f"🎯 CORNIX SIGNAL:",
            f"{cornix_symbol}",
        ...
        # Add Take Profit targets (Official Cornix format)
        target_num = 1
        if tp1 > 0:
            cornix_lines.append(f"Target {target_num}: {tp1:.5f}")
            target_num += 1
        if tp2 > 0:
            cornix_lines.append(f"Target {target_num}: {tp2:.5f}")
            target_num += 1
        if tp3 > 0:
            cornix_lines.append(f"Target {target_num}: {tp3:.5f}")
        
        # Add Stop Loss (Official Cornix format)
        cornix_lines.append(f"")
        cornix_lines.append(f"Stop Loss: {stop_loss:.5f}")
        ...
        lines = strategy_lines + cornix_lines + footer_lines
